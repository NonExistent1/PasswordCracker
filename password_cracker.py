...
'''
-----DESIGN-----
I will solve the problem by going through each possible character for each character in a 4 character password.
To actually solve the problem I did that but also had to take hash into consideration and switch each into hash
and then check if they matched.
I tested to make sure the passwords were printing into what they were supposed to, that it worked with other words,
and that it worked with and without symbols.
My code takes hashcode and then compares it to other hashcode that it found by going through each option for 
each character, and then if it matches, prints out its results.
'''
'''
-----USER DOCUMENTATION-----
Run the program. The program then will crack a predetermined password given by the program. After it finishes that
it will print out its results along with what the results are supposed to be.
'''
'''
Name: Jordyn Kuhn
Date: 10/26/22
...
def crack_password(hash):
    ...
    Crack a 4 character long password (lowercase and symbols).
    """

    #reset the password
    password = None

    #set the variations to lowercase and symbols
    variations = string.ascii_lowercase + string.punctuation

    #loop through every option of letters
    for first_letter in variations:
        for second_letter in variations:
            for third_letter in variations:
                for fourth_letter in variations:
                    #set the password being guessed to a string of the 4 letters
                    password_guess = first_letter + second_letter + third_letter + fourth_letter
                    #change it to hash
                    hash_guess = hashlib.md5(password_guess.encode()).hexdigest()
                    #see if the hash matches
                    if hash_guess == hash:
                        #set it to the password if it does
...
if __name__ == '__main__':
    ...
    #hash_brute_force()
    word = "duck"
    password = crack_password(hashlib.md5(word.encode()).hexdigest())
    print("Cracked password: {} (should be {})".format(password, word))